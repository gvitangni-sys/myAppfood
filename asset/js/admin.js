...
function verifierAuthentificationAdmin() {
  ...
  try {
    ...
    if (userMenuBtn) {
      const userNameElement = userMenuBtn.querySelector(
...
async function chargerStatistiques() {
  try {
    ...
    if (!error.message.includes("Accès refuse")) {
...
document.querySelectorAll(".toggle-checkbox").forEach((checkbox) => {
  checkbox.addEventListener("change", function () {
    console.log("Toggle modifié:", this.checked);
    // Ajouter ici la logique pour sauvegarder les préférences
  });
});
